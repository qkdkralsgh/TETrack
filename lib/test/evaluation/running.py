...
def run_sequence(seq: Sequence, tracker: Tracker, debug=False, viz_attn=False, distill=0, num_gpu=8):
    ...
    if debug:
        output = tracker.run_sequence(seq, debug=debug, viz_attn=viz_attn, distill=distill)
    else:
        try:
            output = tracker.run_sequence(seq, debug=debug, viz_attn=viz_attn, distill=distill)
...
def run_dataset(dataset, trackers, debug=False, viz_attn=False, distill=0, threads=0, num_gpus=8):
    ...
    if mode == 'sequential':
        for seq in dataset:
            for tracker_info in trackers:
                run_sequence(seq, tracker_info, debug=debug, viz_attn=viz_attn, distill=distill)
    elif mode == 'parallel':
        param_list = [(seq, tracker_info, debug, viz_attn, distill, num_gpus) for seq, tracker_info in product(dataset, trackers)]
